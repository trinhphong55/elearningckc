...
router.post('/', async (req, res) => {
  ...
  //neu dsKHDT rong?
  if (dsKHDT.length === 0) {
    return res.json({ error: 'Chuong trinh dao tao nay chua co ke hoach dao tao'})
  }
  ...
  dsLopHoc.forEach(lh => {
    dsKHDT.forEach(khdt => {
      lophocphan = {
        tenLopHocPhan: lh.tenLop, maLopHoc: lh.maLopHoc,
        maDaoTao: khdt.maDaoTao, hocKi
      }
  ...
  await asyncForEach(dsLHP, async (lhp, index) => {
    await LopHocPhan.findOne({ maDaoTao: lhp.maDaoTao, maLopHoc: lhp.maLopHoc }).exec().then((data) => {
      if (data === null) {
  ...
  if (soLopHocPhan === dsLHP.length) {
    return res.json({ success: `Them moi thanh cong ${soLopHocPhan} lop hoc phan` });
  }
