...
_router.get("/danhsachbaitap/:maLopHocPhan", async (req, res) => {
  try {
    var chude = await CHUDE.find({
      maLopHocPhan: req.params.maLopHocPhan,
      trangThai: 1,
    });
    var baitap = await BaiTap.find({
      trangThai: 1,
      lopHocPhan: req.params.maLopHocPhan,
    });
    var cotdiem = await COTDIEM.find({
      trangThai: 1,
      maLopHocPhan: req.params.maLopHocPhan,
    });
    var data = [];
          ...
          if (x.maChuDe == y.chuDe) {
            if (y.maBaiTap == z.maBaiTap) {
              data.push({
                tieuDe: y.tieuDe,
                tenChuDe: x.tenChuDe,
                cotDiem: z.tenCotDiem,
                maCotDiem: z.maCotDiem,
              });
            }
          }
        });
